%Anup Das
%20 September, 2019
%Subjects for each brain region in the Brainnetome atlas
...
for i_BrainReg = 1:123
    
    %Brainnetome PCCPrec indices
    BrainRegions = [2*i_BrainReg-1 2*i_BrainReg];
    ...
    for i_Subject = 1:NSubjects
        ...
        for j_Comb_Ind = 1:length(BrainRegions)
            
            temp1 = temp1 + FindRegionHO(MNIAllSubjectFilledBipolar{i_Subject,1}, roiFile_Brainnetome, BrainRegions(j_Comb_Ind));
    ...
    if (~isempty(SubjInd))
        ...
        SubjIndGlobal_Brainnetome{i_BrainReg,1} = SubjInd;
...
save(strcat(StrSaveSubjIndGlobal,'SubjIndGlobal_Brainnetome_ArtifactRemove.mat'),'SubjIndGlobal_Brainnetome');
