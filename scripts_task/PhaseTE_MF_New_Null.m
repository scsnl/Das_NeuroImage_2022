function [PTE_Rand, dPTE, PTE, delay, Nbins, binsize] = PhaseTE_MF_New_Null(data, delay, binsize);
...
for i_rand = 1:NRand
    ...
    x_perm_1 = randperm(length(phase_data(:,1)));
    phase_perm_1 = phase_data(x_perm_1, 1);
    phase_data(:,1) = phase_perm_1;
    
    x_perm_2 = randperm(length(phase_data(:,2)));
    phase_perm_2 = phase_data(x_perm_2, 2);
    phase_data(:,2) = phase_perm_2;
    ...
    for i=1:N
        for j=1:N
            if i~=j
                ...
                Hypr_y = -nansum(Pypr_y.*log2(Pypr_y), 'all');
                Hy_x = -nansum(Py_x.*log2(Py_x), 'all');
                Hypr_y_x = -nansum(Pypr_y_x.*log2(Pypr_y_x), 'all');
    ...
    PTE_Rand = [PTE_Rand; PTE(1,2)];
    PTE_Rand = [PTE_Rand; PTE(2,1)];
