%Anup Das
%3 November, 2019
%PTE calculation for inter-DMN in the Yeo2011 atlas for 7-Networks in the
...
%save variables
StrSavePTEVals = '/oak/stanford/groups/menon/projects/a1das/2018_ECoG_DMN_Project/scripts/Release_20171012_scripts/scripts_task/PTE_Results/Yeo_atlas/Networks_7/Encode/';
...
for i_interDMN = 1:6
    ...
    if (~isempty(SubjInd))
        ...
        for i=1:size(SubjInd,1)
            ...
            Recalled_Mat = [];
            for i_conv = 1:size(Recalled,1)
                
                Recalled_Mat = [Recalled_Mat; double(Recalled{i_conv,1})];
        ...
        [PTE_12, PTE_21] = PTECalc(RegXData, RegYData);
        PTEVals_12 = [PTEVals_12; PTE_12];
        PTEVals_21 = [PTEVals_21; PTE_21];
       
        save(strcat(StrSavePTEVals,'CorrIncorr_PTEVals_DMNHIPP_',num2str(i_interDMN),'_Yeo2011_ArtifactRemove.mat'),'PTEVals_12');
        save(strcat(StrSavePTEVals,'CorrIncorr_PTEVals_',num2str(i_interDMN),'_DMNHIPP_Yeo2011_ArtifactRemove.mat'),'PTEVals_21');
